...
progress is answered from the ledger rather than recorded as a failure. A read
that fails for a transient reason keeps reporting a still-fresh observation;
only a command's preflight insists on a successful read.
