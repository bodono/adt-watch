...
public final class ArmExperimentService extends Service {
        ...
        MAIN.post(() -> {
            ...
            if (current != null && !current.stopped && !current.finishing) {
                if (toggleRevision != null) {
                    // A phone-prepared diagnostic session takes the watch's single tap as its confirmation;
                    // a routine session never adopts a second, unrelated toggle.
    ...
    /**
     * The "Prepare one test" diagnostic was unreachable: the watch only ever sends a state-bound
     * toggle, which this service dropped while the prepared session was active. Adopting the tap
     * binds the session to the watch's displayed state revision, so the same state gate and
     * durable BUSY bookkeeping apply as for a routine request.
     */
    private boolean adoptWatchTap(String source, ArmExperimentProtocol.Message message, String revision) {
        ...
        if (!PhoneAlarmState.matches(this, revision, action)) { PhoneStateLink.publish(this); return true; }
